...
Advanced Tailwind CSS v4 patterns including animations, dark mode theming, custom utilities, theme modifiers, namespace overrides, and the v3-to-v4 migration checklist. In this Laravel React project, use these native CSS animation patterns for simple decorative or state-attribute entry/exit effects; use Motion for interactive, gesture-driven, layout, or stateful animation.
...
- [ ] Remove unnecessary `forwardRef` usage. Keep it when a library contract
      requires a DOM ref, such as Motion `motion.create()` or Radix slot
      children.
...
- **Don't use `forwardRef` by default** - React 19 passes ref as prop. Keep it
  only when a library contract requires it.
- **Don't reach for arbitrary values first** - Use existing tokens first and extend
  `@theme` only when reuse across components is likely. Rare local arbitrary
  values are acceptable when justified.
