...
Do not run installation commands unless the user explicitly approves dependency
...
```tsx
// Component.tsx
<Dialog.Content className="rounded-[var(--radius-base)] bg-[hsl(var(--color-surface))] shadow-[var(--shadow-lg)]" />
```
    ...
    'inline-flex items-center justify-center rounded-md font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50',
    {
        variants: {
            variant: {
                ...
                outline: 'border-input bg-background border hover:bg-accent',
...
**Use case**: Render as different elements without losing behavior when
composition or polymorphism actually requires it. Do not expose `asChild` by
default on every primitive.
...
**Why `asChild` matters**: Prevents nested button/link issues in the accessibility tree when consumers need to provide the rendered element.
...
1. **Use `asChild` when polymorphic rendering or composition needs it**
...
Do not run these commands unless the user explicitly approves dependency or
