...
> **Project overrides for this Laravel Inertia app:**  
> Apply React, browser, bundle, hydration, and Vite guidance directly. Treat
> Next.js, RSC, Server Action, and App Router rules as informational unless
...
4. [Client-Side Data Fetching](#4-client-side-data-fetching) — **MEDIUM-HIGH**
    ...
    - 4.3 [Deduplicate Client Requests](#43-deduplicate-client-requests)
...
For operations with more complex dependency chains, use the no-new-dependency promise pattern from Dependency-Based Parallelization. Use `better-all` only if it is already installed or the user explicitly approves adding it.
...
### 4.3 Deduplicate Client Requests

**Impact: MEDIUM-HIGH (automatic deduplication)**

SWR enables request deduplication, caching, and revalidation across component instances. In this Laravel Inertia app, use SWR only if it is already installed or the user explicitly approves adding it. Otherwise prefer Inertia `useHttp`, shared page props, or a small local request cache.
...
**Correct when SWR is already installed or approved: multiple instances share one request**
